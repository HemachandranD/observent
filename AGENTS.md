...
1. **Spec** — Run both detectors **in parallel terminal calls** (deterministic JSON producers — do not wrap in a subagent):
   ...
   ```
   Resolve framework + backend(s) (1–5 from the list). Convention is **derived mechanically** from the backend set (`{phoenix}` → `oi`; non-Phoenix subset → `otel-genai`; mixed → `both`). Capture the existing-setup decision (extend / replace / abort). **Self-host provisioning offer:** for each backend whose endpoint is `self-host` and unreachable, if it's one of `{phoenix, langfuse, signoz, elastic-apm}` and Docker + Compose are available, ask `Provision it locally with Docker? (yes / no, I'll start it myself / skip)` and record the choice (LangSmith is never offered — enterprise-only; see `${OBSERVENT_HOME}/references/self_host.md`). Write `.observent/spec.md` with `status: locked`.
2. **Plan** — Read `spec.md`. Using `${OBSERVENT_HOME}/references/matrix.md`, write `.observent/plan.md` with the full generated content of every file in anchored fenced blocks (`<!-- plan:<slug> -->`). For each opted-in self-host backend, materialize its stack from `${OBSERVENT_HOME}/references/self_host.md` into a `plan.provision[]` entry (`vendored-compose` → a `docker-compose.observent-<backend>.yml` block; `upstream-clone` → a pinned `git clone … && docker compose … up` command), copying image tags verbatim. Set `spec_fingerprint`.
3. **Tasks** — Decompose `plan.md` into `.observent/tasks.json` per the canonical schema: `confirm` (diff preview) → `write_file`(s) → `edit_file`(s) → `run_command` (pip install) → `run_command` (one per `plan.provision[]` `docker compose … up`, between pip-install and validate) → `validate` (last). Set `plan_fingerprint`. All `status: pending`.
...
- Backend init from env vars (never hard-coded keys); framework instrumentor or native trace processor; multi-agent attrs keyed by convention; baggage (`session.id`, `user.id`, `tenant.id`, `app.version`); flush-on-exit via `atexit`; OTLP **HTTP** not gRPC; W3C default propagator (never call `set_global_textmap()`).
...
References: `${OBSERVENT_HOME}/SKILL.md` · `${OBSERVENT_HOME}/references/spec_schema.md` · `${OBSERVENT_HOME}/references/matrix.md` · `${OBSERVENT_HOME}/references/openinference.md` · `${OBSERVENT_HOME}/references/otel_genai.md` · `${OBSERVENT_HOME}/references/examples.md` · `${OBSERVENT_HOME}/references/capture.md` · `${OBSERVENT_HOME}/references/self_host.md`.
